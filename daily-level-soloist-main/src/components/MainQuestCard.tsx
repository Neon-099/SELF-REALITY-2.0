...
const AddTaskDialog = ({ questId, onClose }: { questId: string; onClose: () => void }) => {
  ...
  const handleAddTask = () => {
    ...
    // Call addQuestTask with the correct parameters
...
const MainQuestCard: React.FC<MainQuestCardProps> = ({ quest, onComplete, onStart, canComplete }) => {
  ...
  // Get the latest quest data from the store
  const getLatestQuestData = () => {
    const latestQuest = quests.find(q => q.id === quest.id);
    if (latestQuest) {
      console.log('Latest quest data:', latestQuest);
      return latestQuest;
    }
    return quest;
  };
  ...
  const handleStartQuest = async () => {
    try {
      // Log before starting
      console.log('Before starting quest:', quest.id);
      console.log('Quest tasks before starting:', quest.tasks);

      // Start the quest first and get the updated quest data
      const startedQuest = await onStart(quest.id);

      // Log after starting
      console.log('After starting quest, received data:', startedQuest);

      // Force a refresh of the quest data from the store
      const latestQuest = getLatestQuestData();
      console.log('Latest quest data after starting:', latestQuest);
      console.log('Latest quest tasks:', latestQuest.tasks);

      // Wait a moment for the state to update
      setTimeout(() => {
        // Get the latest data again after a short delay
        const refreshedQuest = getLatestQuestData();
        console.log('Refreshed quest data after timeout:', refreshedQuest);
        console.log('Refreshed quest tasks:', refreshedQuest.tasks);

        // Open the task dialog
        setIsTaskDialogOpen(true);
      }, 100);
    } catch (error) {
      console.error('Error starting quest in MainQuestCard:', error);
      toast({
  ...
  const handleCardClick = () => {
    if (quest.started && !quest.completed) {
      // Log for debugging
      const latestQuest = getLatestQuestData();
      console.log('Card clicked, latest quest data:', latestQuest);
      console.log('Tasks in latest quest:', latestQuest.tasks);

      setIsTaskDialogOpen(true);
    }
  };

  // Check if all tasks are completed using the latest quest data
  const areAllTasksCompleted = () => {
    const latestQuest = getLatestQuestData();
    return latestQuest.tasks && latestQuest.tasks.length > 0 &&
      latestQuest.tasks.every(task => task.completed);
  };
      ...
      {!quest.started ? (
        ...
        >
            ...
            <div className="py-1.5 flex-1 overflow-hidden flex flex-col">
              ...
              {(() => {
                const latestQuest = getLatestQuestData();
                console.log('Rendering tasks in dialog:', latestQuest.tasks);
                return null;
              })()}

              {(() => {
                // Get the latest quest data right before rendering
                const latestQuestData = getLatestQuestData();
                console.log('Rendering dialog with latest quest data:', latestQuestData);
                console.log('Tasks to render:', latestQuestData.tasks);

                return latestQuestData.tasks && latestQuestData.tasks.length > 0 ? (
                  <div className="space-y-1.5 flex-1 overflow-y-auto pr-1.5 custom-scrollbar">
                    {latestQuestData.tasks.map((task) => (
                    <div
                      key={task.id}
                        ...
                        {task.completed ? (
                          ...
                          <div
                            onClick={(e) => {
                              ...
                              if(!task.completed) {
                                const latestQuest = getLatestQuestData();
                                completeQuestTask(latestQuest.id, task.id);
                              }
                            }}
              ...
              {getLatestQuestData().tasks &&
               getLatestQuestData().tasks.length > 0 &&
               getLatestQuestData().tasks.every(task => task.completed) ? (
                <div className="flex justify-center mt-2">
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      const latestQuest = getLatestQuestData();
                      onComplete(latestQuest.id, latestQuest.title, latestQuest.expReward);
                      setIsTaskDialogOpen(false);
                    }}
            ...
            >
              ...
              <div className="py-1.5 flex-1 overflow-hidden flex flex-col">
                ...
                {(() => {
                  // Get the latest quest data right before rendering
                  const latestQuestData = getLatestQuestData();
                  console.log('Rendering second dialog with latest quest data:', latestQuestData);
                  console.log('Tasks to render in second dialog:', latestQuestData.tasks);
